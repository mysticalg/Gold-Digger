...
/**
 * Spawn a short block-removal animation at a world tile.
 * type: 'dig' for regular mining, 'blast' for bombs.
 */
function addDigEffect(x, y, material, type = 'dig') {
  state.digEffects.push({
    x,
    y,
    material,
    type,
    startedAt: performance.now(),
    durationMs: type === 'blast' ? 320 : 220,
  });
  startEffectLoop();
}

/**
 * Drives temporary dig animations without forcing the whole game into a full-time render loop.
 */
function startEffectLoop() {
  if (state.effectLoopActive) return;
  state.effectLoopActive = true;

  const tick = () => {
    if (state.digEffects.length === 0) {
      state.effectLoopActive = false;
      return;
    }
    draw();
...
/** Render lightweight digging/breaking animations over the map. */
function renderDigEffects(tileSize, offsetX, offsetY, viewportCols, viewportRows, frameNow) {
  state.digEffects = state.digEffects.filter((effect) => (frameNow - effect.startedAt) < effect.durationMs);

  for (const effect of state.digEffects) {
    const progress = (frameNow - effect.startedAt) / effect.durationMs;
    const sx = effect.x - state.camera.x;
    const sy = effect.y - state.camera.y;
    if (sx < -1 || sy < -1 || sx > viewportCols || sy > viewportRows) continue;

    const px = offsetX + (sx * tileSize);
    const py = offsetY + (sy * tileSize);
    const color = getMaterialColor(effect.material);
    const alpha = Math.max(0, 1 - progress);

    // Shrinking center core to feel like a block collapsing.
    const coreSize = Math.max(2, Math.floor(tileSize * (0.85 - (progress * 0.6))));
    const coreX = px + Math.floor((tileSize - coreSize) / 2);
    const coreY = py + Math.floor((tileSize - coreSize) / 2);
    ctx.fillStyle = color;
    ctx.globalAlpha = Math.min(1, alpha * 0.95);
    ctx.fillRect(coreX, coreY, coreSize, coreSize);

    // Outer pulse ring for impact feedback.
    const pulseSize = Math.floor(tileSize * (0.35 + (progress * 1.05)));
    const pulseX = px + Math.floor((tileSize - pulseSize) / 2);
    const pulseY = py + Math.floor((tileSize - pulseSize) / 2);
    ctx.strokeStyle = effect.type === 'blast' ? '#ff8f5a' : '#fff2b0';
    ctx.globalAlpha = Math.max(0, alpha * (effect.type === 'blast' ? 0.9 : 0.65));
...
function draw() {
  ...
  // Overlay short dig/remove animations after terrain drawing.
  renderDigEffects(tileSize, offsetX, offsetY, viewportCols, viewportRows, performance.now());
...
function useBomb() {
  ...
  for (let y = originY - 1; y <= originY + 1; y += 1) {
    for (let x = originX - 1; x <= originX + 1; x += 1) {
      if (!inBounds(x, y)) continue;
      const block = getCell(x, y);
      if (block !== MATERIALS.EMPTY) addDigEffect(x, y, block, 'blast');
      setCell(x, y, MATERIALS.EMPTY);
      settleColumn(x);
    }
  }
